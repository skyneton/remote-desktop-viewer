...
namespace RemoteDesktopViewer.Network
{
    public static class ScreenThreadManager
    {
        ...
        private static void SendScreenChunk()
        {
            ...
            for (var y = 0; y < sizeY; y++)
            {
                for (var x = 0; x < sizeX; x++)
                {
                    ...
                    using (var split = _bitmap.Clone(new Rectangle(posX, posY, width, height), PixelFormat.DontCare))
                    {
        ...
        private static Bitmap TakeDesktop()
        {
            ...
            {
                graphics.CopyFromScreen(size.X, size.Y, 0, 0, size.Size, CopyPixelOperation.SourceCopy);
                return bitmap;
            }
        }
